...
Item {
  ...
  // --- Drawer (hover-reveal widget host) -----------------------------------
  //
  // A widget hosted in the drawer never leaves its section's bar.layout
  // array — it just carries an inline `drawer: true` (and optionally
  // `drawerHidden: true`) setting, same shape as any other widget's inline
  // settings (a clock's `format`, say). That's what lets host/unhost/hide
  // reuse the ordinary mutateShellConfig write path below with no relocation
  // bookkeeping: the entry's position never needs to be remembered, because
  // it never moves.
  //
  // Hosting is restricted to the drawer's own section (see drawerSection):
  // reordering *within* the drawer is then a plain adjacent swap inside one
  // array, the same as reordering any two ordinary bar widgets, with no
  // cross-section ordering to invent.
  readonly property var drawerConfig: Util.isPlainObject(barConfig) && Util.isPlainObject(barConfig.drawer)
    ? barConfig.drawer : ({})

  // left or right only — the center section already has its own anchor
  // splitting (centerAnchor) and combining the two isn't worth the added
  // complexity for a feature whose whole point is staying simple.
  readonly property string drawerSection: {
    var serial = barConfigSerial
    var section = String(drawerConfig.section || "right")
  ...
  // Drawer-hosted entries, in the order they sit in drawerSection's array.
  function drawerEntries() {
    var entries = root.layoutEntries(root.drawerSection)
    var out = []
    for (var i = 0; i < entries.length; i++) {
      if (BarModel.isDrawerEntry(entries[i])) out.push(entries[i])
    }
  ...
  function drawerHostedIds() {
    return root.drawerEntries().map(function(entry) { return root.entryId(entry) })
  }

  // Hosted entries actually shown in the reveal row (hosted minus hidden) —
  // there is no third state, same as the tray's own pinned/drawer split.
  function drawerVisibleEntries() {
    var out = []
    var entries = root.drawerEntries()
    for (var i = 0; i < entries.length; i++) {
      if (!BarModel.isDrawerHiddenEntry(entries[i])) out.push(entries[i])
    }
    return out
  }

  function candidateDrawerWidgets() {
    var registry = root.barWidgetRegistry
    if (!registry) return []
    return BarModel.candidateDrawerWidgets(
      registry.availableIds ? registry.availableIds() : Object.keys(registry.widgets || {}),
      function(id) { return registry.metadataFor ? registry.metadataFor(id) : null },
      root.drawerHostedIds()
    )
  }

  // Ensures `id`'s entry lives in drawerSection (moving it there from
  // wherever it currently is, or inserting a fresh one at the section's end
  // if it isn't placed anywhere yet) and flags it drawer:true. One
  // structural write, same shape as an ordinary drag-reorder.
  function hostInDrawer(id) {
    var widgetId = String(id || "")
    if (!widgetId || BarModel.EXCLUDED_DRAWER_IDS.indexOf(widgetId) !== -1) return
    if (!root.shell || typeof root.shell.mutateShellConfig !== "function") return
    root.shell.mutateShellConfig(function(config) {
      var section = root.rawLayoutSection(config, root.drawerSection)
      var idx = root.rawEntryIndex(section, widgetId)
      var entry
      if (idx === -1) {
        // Not in drawerSection yet — pull it out of wherever it currently
        // lives (any other section), or start fresh if it was never placed.
        var sections = ["left", "center", "right"]
        var moved = null
        for (var s = 0; s < sections.length; s++) {
          if (sections[s] === root.drawerSection) continue
          var other = root.rawLayoutSection(config, sections[s])
          var otherIdx = root.rawEntryIndex(other, widgetId)
          if (otherIdx !== -1) {
            moved = other[otherIdx]
      ...
      entry.drawer = true
      delete entry.drawerHidden
    })
  }

  // Clears the drawer flags in place — the entry stays exactly where it is
  // in drawerSection and simply renders in the normal strip there next.
  function unhostFromDrawer(id) {
    var widgetId = String(id || "")
    if (!widgetId || !root.shell || typeof root.shell.mutateShellConfig !== "function") return
    root.shell.mutateShellConfig(function(config) {
      var section = root.rawLayoutSection(config, root.drawerSection)
      var idx = root.rawEntryIndex(section, widgetId)
      if (idx === -1) return
      var entry = section[idx]
  ...
  function setDrawerEntryHidden(id, hidden) {
    var widgetId = String(id || "")
    if (!widgetId || !root.shell || typeof root.shell.mutateShellConfig !== "function") return
    root.shell.mutateShellConfig(function(config) {
      var section = root.rawLayoutSection(config, root.drawerSection)
      ...
      var entry = section[idx]
      if (typeof entry !== "object" || entry === null) return
      if (hidden) entry.drawerHidden = true
      else delete entry.drawerHidden
    })
  }

  // Swaps id with its drawer-order neighbor. The two entries may not be
  // physically adjacent in drawerSection's array (ordinary, non-hosted
  // widgets can sit between them) — swapping their *values* at their own
  // absolute indices reorders them relative to each other without moving
  // anything else, since the strip only ever reads the non-hosted entries
  // and does not care what sits at the indices in between.
  function moveDrawerEntry(id, direction) {
    var widgetId = String(id || "")
    if (!widgetId || !root.shell || typeof root.shell.mutateShellConfig !== "function") return
    root.shell.mutateShellConfig(function(config) {
      var section = root.rawLayoutSection(config, root.drawerSection)
      ...
      for (var i = 0; i < section.length; i++) {
        if (BarModel.isDrawerEntry(section[i])) hostedIndexes.push(i)
      }
      var pos = -1
      for (var h = 0; h < hostedIndexes.length; h++) {
        if (root.entryId(section[hostedIndexes[h]]) === widgetId) { pos = h; break }
      }
      var swapWith = pos + (direction < 0 ? -1 : 1)
      if (pos === -1 || swapWith < 0 || swapWith >= hostedIndexes.length) return
      var ia = hostedIndexes[pos], ib = hostedIndexes[swapWith]
      var tmp = section[ia]
  ...
  function findCenterAnchorEntry() {
    var entries = root.stripEntries("center")
    var idx = root.entryIndex(entries, root.centerAnchor)
    return idx === -1 ? null : entries[idx]
  }

  // The literal, never-persisted id for the drawer's own glyph slot, spliced
  // into a section's rendered entries at render time only (see
  // sectionEntriesForRender) — never written to shell.json, so ModuleSlot's
  // drag handling (which looks entries up by id in the real config) just
  // no-ops if someone tries to drag it, and un-hosting every widget makes it
  // vanish from the strip exactly like any other empty section would.
  readonly property string drawerEntryId: "kc.omarchy-menubar-manager.drawer"

  // A section's strip entries, with the drawer glyph inserted right after
  // tray (or at the front, if tray isn't in this section) when this is
  // drawerSection. Every other section renders untouched.
  function sectionEntriesForRender(region) {
    var entries = root.stripEntries(region)
    if (region !== root.drawerSection) return entries
    var out = entries.slice()
    var trayIdx = root.entryIndex(out, "omarchy.tray")
    out.splice(trayIdx === -1 ? 0 : trayIdx + 1, 0, { id: root.drawerEntryId })
    return out
  }

  component LeftModules: ModuleList {
    entries: root.sectionEntriesForRender("left")
    region: "left"
  }

  component RightModules: ModuleList {
    entries: root.sectionEntriesForRender("right")
    region: "right"
  }
  ...
  component CenterModules: Item {
    ...
    property var entries: root.stripEntries("center")
