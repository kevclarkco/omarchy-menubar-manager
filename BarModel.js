...
function inlineSettingsDelta(current, next) {
  ...
  for (var s = 0; s < regions.length; s++) {
    ...
    for (var j = 0; j < a.length; j++) {
      ...
      // performance point), which leaves those bindings stale. Force the
      // full rebuild path (return null) for both flags so they recompute.
      if (isDrawerEntry(a[j]) !== isDrawerEntry(b[j])) return null
      if (isDrawerHiddenEntry(a[j]) !== isDrawerHiddenEntry(b[j])) return null
...
// A layout entry hosted in the drawer instead of its section's normal strip.
function isDrawerEntry(entry) {
  return entrySettings(entry).drawer === true
}

// A drawer-hosted entry that's tracked but not shown in the reveal row.
function isDrawerHiddenEntry(entry) {
...
// registered bar widget minus what's already drawer-hosted minus
// EXCLUDED_DRAWER_IDS, sorted by display name.
function candidateDrawerWidgets(availableIds, metadataForFn, hostedIds) {
  var out = []
  for (var i = 0; i < availableIds.length; i++) {
    var id = availableIds[i]
    if (hostedIds.indexOf(id) !== -1) continue
    if (EXCLUDED_DRAWER_IDS.indexOf(id) !== -1) continue
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    isDrawerEntry: isDrawerEntry,
    isDrawerHiddenEntry: isDrawerHiddenEntry,
    EXCLUDED_DRAWER_IDS: EXCLUDED_DRAWER_IDS,
    candidateDrawerWidgets: candidateDrawerWidgets,
